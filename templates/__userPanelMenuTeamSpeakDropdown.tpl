{if HANASHI_TEAMSPEAK_VIEWER_SHOW_MENU}
<li id="teamspeakUsers" data-count="{#$__wcf->getTeamSpeakViewerMenuHandler()->getClientCount()}">
    <a class="jsTooltip" href="{link controller='TeamSpeakViewer'}{/link}" title="{lang}wcf.page.teamSpeakViewer.title{/lang}">
        ...
        <span>{lang}wcf.page.teamSpeakViewer.title{/lang}</span>
        {if $__wcf->getTeamSpeakViewerMenuHandler()->getClientCount() > 0}<span class="badge badgeUpdate">{#$__wcf->getTeamSpeakViewerMenuHandler()->getClientCount()}</span>{/if}
    </a>

    <script data-relocate="true" src="{@$__wcf->getPath()}js/WCF.TeamSpeak.js?v={@LAST_UPDATE_TIME}"></script>
